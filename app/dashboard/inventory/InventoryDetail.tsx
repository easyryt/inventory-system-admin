    ...
    <div className="space-y-5">
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <div className="mt-5 grid grid-cols-2 gap-3 lg:grid-cols-4">
          ...
          <div
            className={`rounded-xl p-3 ${lowStockRows ? "bg-rose-50" : "bg-slate-50"}`}
          >
            <p
              className={`text-[11px] font-medium uppercase tracking-wide ${lowStockRows ? "text-rose-800" : "text-slate-600"}`}
            >
              Needs attention
            </p>
            <p
              className={`mt-1 text-xl font-semibold ${lowStockRows ? "text-rose-950" : "text-slate-900"}`}
            >
              {lowStockRows}
            </p>
          </div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 px-4 py-4 sm:px-5">
          <div>
            ...
            <p className="mt-0.5 text-xs text-slate-500">
              Edit minimum thresholds here. Design-level stock is shown
              separately from RAW model stock.
            </p>
          </div>
          <span className="text-xs text-slate-500">
            {totalBarcodes} barcode labels created
          </span>
        </div>
                ...
                >
                  <div className="flex min-w-0 flex-1 items-center gap-3">
                    {row.type === "PRINTED" && row.designUrl ? (
                      ...
                      <span
                        className={`grid h-16 w-16 shrink-0 place-items-center rounded-lg px-1 text-center text-[10px] font-semibold ${row.type === "RAW" ? "bg-amber-100 text-amber-800" : "bg-slate-100 text-slate-500"}`}
                      >
                        {row.type}
                      </span>
                    )}
